...
export default function PlayerInfosSelection() {
	...
	useEffect(() => {
		if (account) {
			setName(account.address.slice(0, 6));
		} else {
			setName('Player');
		}
	}, [account]);
		...
		<>
			...
			<main className='space-y-6 px-6 pb-6 pt-4 sm:px-8 sm:pb-8 sm:pt-6'>
				<div className="flex flex-col items-center justify-center">
					<label htmlFor='nameInput' className='block space-y-2 text-center mb-4'>
						<span className='block text-center text-lg font-bold text-gray-900 dark:text-white'>
							{account ? `Welcome, ${account.address.slice(0, 6)}!` : "Please Connect Wallet to Play (Top Right)"}
						</span>
					</label>
				</div>
				<div>
				...
				<Button
					...
					disabled={!account}
				>
					{account ? "Save to play" : "Wallet Required"}
				</Button>
