...
export default function Connected() {
	...
	const {code, peers, maxNbPlayers, network, isWager} = useNetwork();
	const [waitingForHost, setWaitingForHost] = useState(false);

	const isHost = network?.isHost ?? false;

	// Listen for 'game:start' signal from host (for wager mode)
	useEffect(() => {
		if (!network || !isWager || isHost) return;

		const channel = network.channel('game:start' as any, false);
		channel.on(() => {
			navigate('/game');
		});

		setWaitingForHost(true);
	}, [network, isWager, isHost]);
	...
	return (
		<div className='space-y-4 p-6 sm:p-8 md:space-y-4'>
			...
			{/* Wager: only host can start; guests wait */}
			{isWager && !isHost ? (
				<div className='text-center text-gray-400 py-3 animate-pulse'>
				...
				<Button
					...
					size='large'
				>
					{isWager ? 'Start Wager Match' : 'Play'}
				</Button>
			)}
		</div>
	);
